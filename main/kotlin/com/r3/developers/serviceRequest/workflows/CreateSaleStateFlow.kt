...
import com.r3.developers.serviceRequest.models.NewSaleModel
...
@InitiatingFlow(protocol = "Create-Sale-state")
class CreateSaleStateFlow : ClientStartableFlow {
    ...
    override fun call(requestBody: ClientRequestBody) :String {
        val request = requestBody.getRequestBodyAs(jsonMarshallingService, NewSaleModel::class.java)



        val uuid = UUID.randomUUID()
        val saleTransactionDate = Instant.now().toEpochMilli()

      // val arrayOfRequestIds = request.serviceRequestIDList
        val arrayOfBatchIds = request.batchIDList
        val notary = notaryLookup.notaryServices.single()



        val filteredBatchIds = mutableListOf<UUID>()
        for (batchId in arrayOfBatchIds) {
            val serviceBatchStateAndRef = utxoLedgerService.findUnconsumedStatesByType(BatchState::class.java)
                .firstOrNull { stateAndRef ->
                    stateAndRef.state.contractState.batchId == batchId
                }


            if (serviceBatchStateAndRef != null) {
                // RequestId matches the filter criteria, add it to the filtered list
                filteredBatchIds.add(batchId)
                val inputBatchRequestStateRef = serviceBatchStateAndRef.ref
                val outputBatchRequestStateData = serviceBatchStateAndRef.state.contractState

                val totalBoxesRemaining = outputBatchRequestStateData.totalNoOfBoxes - request.totalNoOfBoxesSold

                // Check if the total number of boxes remaining is greater than or equal to the total number of boxes sold
                if (totalBoxesRemaining < request.totalNoOfBoxesSold) {
                    throw IllegalArgumentException("Insufficient boxes remaining for sale")
                }
                else{
                    println("Total number of boxes remaining: $totalBoxesRemaining")
                }
               outputBatchRequestStateData.recordedOn = Instant.now().toEpochMilli()
                outputBatchRequestStateData.status = "created-sale-request"
        ...
        val saleRequest = SaleState(

            creationData = request,

          batchIDList = filteredBatchIds,
            status = "created-sale-request",
            saleId = uuid,



            updatedOn = Instant.now().toEpochMilli(),
           // confirmPaymentDate = Instant.now().toEpochMilli(),
           // saleTransactionDate = Instant.now().toEpochMilli(),
            harvester= harvester.ledgerKeys.first(),
            lsp = lsp.ledgerKeys.first(),
            collector = collector.ledgerKeys.first(),
            broker = broker.ledgerKeys.first(),
        ...
        val transaction = utxoLedgerService.createTransactionBuilder()
            ...
            .addCommand(SaleContract.Create())
            .setTimeWindowUntil(Instant.now().plus( 1, ChronoUnit.DAYS))
            .addSignatories(saleRequest.participants)
            .toSignedTransaction()
...
@InitiatedBy(protocol = "Create-Sale-state")
class saleStatetResponderFlow: ResponderFlow {
