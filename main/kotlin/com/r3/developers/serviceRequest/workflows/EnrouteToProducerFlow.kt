...
import com.r3.developers.serviceRequest.models.EnrouteToProducerModel
...
@InitiatingFlow(protocol = "Enroute-Service-request")
class EnrouteToProducerFlow : ClientStartableFlow {
    ...
    override fun call(requestBody: ClientRequestBody): String {
        val request = requestBody.getRequestBodyAs(jsonMarshallingService, EnrouteToProducerModel::class.java)

        val requestId = request.requestId
        val enrouteToProducerOn = Instant.now().toEpochMilli()
      //  val batchId= request.batchId
        ...
        outputServiceRequestStateData.enrouteToProducerModel= request
        outputServiceRequestStateData.updatedOn = Instant.now().toEpochMilli()
      //  outputServiceRequestStateData.enrouteToProducerOn=Instant.now().toEpochMilli()
        outputServiceRequestStateData.status="enrouted-to-producer"
        ...
        val transaction = utxoLedgerService.createTransactionBuilder()
            ...
            .addCommand(ServiceRequestContract.EnrouteToProducer())
...
@InitiatedBy(protocol = "Enroute-Service-request")
class EnrouteToProducerResponderFlow: ResponderFlow {
